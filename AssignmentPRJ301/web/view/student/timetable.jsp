                                ...
                                <c:forEach items="${requestScope.s.groups}" var="g">
                                    <c:forEach items="${g.sessions}" var="ses">
                                        <c:if test="${ses.date eq d and ses.slot.id eq slot.id}">
                                            <a href="list?tid=${ses.group.course.dept.term.id}&did=${ses.group.course.dept.id}&cid=${ses.group.course.id}&gid=${ses.group.id}">${ses.group.name}</a>
                                            - ${g.course.code} <br/>
                                            ${ses.lecturer.name}<br>
                                            at ${ses.room.name} <br/>
                                            <c:if test="${ses.status}">
                                                <span style="color: #008300;">(Attended)</span>
                                            </c:if>
